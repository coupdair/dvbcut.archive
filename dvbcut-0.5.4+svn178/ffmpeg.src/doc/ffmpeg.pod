...
Set frame size. The format is B<wxh> (avserver default = 160x128, ffmpeg default = same as source).
...
the F<presets> directory in the Libav source tree for examples.
...
directories F<$AVCONV_DATADIR> (if set), and F<$HOME/.avconv>, and in
the datadir defined at configuration time (usually F<PREFIX/share/avconv>)
...
The option C<-codecs> of the av* tools will display the list of
enabled encoders.
...
F<split2.mpeg>, F<split3.mpeg> with F<avplay> use the
command:
...
For example to read with F<avplay> a multimedia resource named
...
To play the same stream using F<avplay>:
...
The following options (set on the B<avconv>/F<avplay> command
...
When watching multi-bitrate Real-RTSP streams with F<avplay>, the
...
is recognized by the C<-vf> and C<-af> options in B<avconv>
and B<avplay>, and by the C<av_parse_graph()> function defined in
...
I<luma_power>:I<luma_radius>:I<chroma_radius>:I<chroma_power>:I<alpha_radius>:I<alpha_power>
...
It accepts the following parameters:
I<width>:I<height>:I<pix_fmt_string>:I<timebase_num>:I<timebase_den>:I<sample_aspect_ratio_num>:I<sample_aspect_ratio.den>
...
Both sources accept an optional sequence of I<key>=I<value> pairs,
